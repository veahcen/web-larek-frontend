...
- set galery(items: HTMLElement[]): void - етод для установки карточек на странице.
...
- set content(value: HTMLElement) - метод для установки контента в модальном окне.
...
- \_total: HTMLElement - элемент разметки c общей стоимостью заказа.
  -\_ button: HTMLButtonElement - элемент разметки с кнопкой оформления заказа.
...
- \_paymentContainer: HTMLDivElement - элемент разметки с методами оплаты.
- \_paymentButtons: HTMLButtonElement[] - массив с элементами кнопок.

Методы:

- setClassPaymentMethod(method: string): void - выбор метода оплаты для выделения кнопки
...
`card:changed` - добавляем/удаляем лот из корзины
...
`order_contacts:open` - следующий пункт оформление заказа
`contacts:submit` - заполнили вторую форму
